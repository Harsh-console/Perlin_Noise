...
float Noise2D(const vector2 &point__, const int GRID_SIZE, const vector<vector<vector2>>& grid){
    ...
    del_v1.x = x - x0; del_v1.y = y - y0;
    del_v2.x = x - x1; del_v2.y = y - y0;
    del_v3.x = x - x0; del_v3.y = y - y1;
    del_v4.x = x - x1; del_v4.y = y - y1;
...
int main(int argv, char* argc[]){
    ...
    for(int i = 0; i < WIDTH - 1; i++){
        for(int j = 0; j < HEIGHT - 1; j++){
            ...
            SDL_SetRenderDrawColor(renderer, pix_brightness, pix_brightness, pix_brightness, 255);
            rect.x = j;
            rect.y = i;
